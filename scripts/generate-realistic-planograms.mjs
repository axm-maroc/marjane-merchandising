...
// Configuration des planogrammes réalistes par magasin
const planogramConfigs = [
  {
    storeId: 1,
    planograms: [
      {
...
async function generateRealisticPlanograms() {
  let connection;
  try {
    ...
    console.log('\n📊 Génération de planogrammes réalistes...\n');

    let totalCreated = 0;
    let totalProducts = 0;

    for (const storeConfig of planogramConfigs) {
      const storeId = storeConfig.storeId;
      
      // Récupérer les emplacements du magasin
      const [locations] = await connection.execute(
        'SELECT id FROM planogramLocations WHERE storeId = ? LIMIT 3',
        [storeId]
      );

      if (locations.length === 0) {
        console.log(`⚠️  Magasin ${storeId} : Aucun emplacement trouvé`);
        continue;
      }

      console.log(`\n🏪 Magasin ${storeId}:`);

      for (let i = 0; i < storeConfig.planograms.length; i++) {
        const planogramConfig = storeConfig.planograms[i];
        ...
        const [result] = await connection.execute(
          `INSERT INTO planograms (locationId, name, version, status, createdAt) 
           VALUES (?, ?, 1, ?, NOW())`,
          [locationId, planogramConfig.name, planogramConfig.status]
        );

        const planogramId = result.insertId;

        // Ajouter les produits
        for (let j = 0; j < planogramConfig.productIds.length; j++) {
          const productId = planogramConfig.productIds[j];
          const quantity = planogramConfig.quantities[j];
        ...
        console.log(`  ✅ ${planogramConfig.name} (${planogramConfig.productIds.length} produits, ${planogramConfig.status})`);
        totalCreated++;
      }
    }

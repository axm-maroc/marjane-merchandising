...
// Produits réalistes pour remplir les planogrammes
...
async function enrichPlanograms() {
  try {
    ...
    for (const planogram of planograms) {
      ...
      // Déterminer la catégorie basée sur le nom de l'emplacement
      ...
      console.log(`✅ Planogramme "${planogram.name}" enrichi avec ${products.length} produits`);
    }

    console.log(`\n📦 Total: ${productsAdded} produits ajoutés aux planogrammes`);
